...
| 12 | `ERROR_REASONS` is hand-duplicated: once in `scripts/galley_normalize.py` (drives `summary.errorPrinters`) and once in `Model.js` (drives the bar icon and card colors). Edit one without the other and you get a red printer card sitting next to a "0 errors" tooltip. | `CrossLanguageErrorReasonsTest.test_javascript_error_reasons_match_python`, which parses both source files with a regex and diffs the sets. |
